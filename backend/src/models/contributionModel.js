 ...
 * Always returns 12 months, even if there were no contributions.
 *
 * Output example:
 * { month: "2025-01", in_total: "1200", tx_count: "5" }
 */
...
export async function updateContribution(contributionId, actorUserId, { amount, paid_at, note }) {
  ...
  try {
    ...
    const beforeRes = await client.query(
      `SELECT id, group_id, user_id, amount, paid_at, note, created_at, updated_at, updated_by, deleted_at, deleted_by
       FROM contributions
       WHERE id = $1
       LIMIT 1`,
      [contributionId]
    ...
    const updatedRes = await client.query(
      `UPDATE contributions
       SET amount = $1,
           ...
           updated_by = $4
       WHERE id = $5
       RETURNING id, group_id, user_id, amount, paid_at, note, created_at, updated_at, updated_by`,
    ...
    const delRes = await client.query(
      `UPDATE contributions
       SET deleted_at = NOW(),
           ...
           delete_reason = $2
       WHERE id = $3
       RETURNING id, group_id, user_id, amount, paid_at, note, created_at, deleted_at, deleted_by, delete_reason`,
